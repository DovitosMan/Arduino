...
/*
  Версия 1.8
  - Добавлен режим для вспышки, контакты с фотика на GND и A1 (обычная схема) или GND и D6 (печатная плата)
*/

// ================ НАСТРОЙКИ ================
// ------ Схема ------
#define WIRING_VERSION 0    // тип схемы. 0 - обычная, 1 - печатная плата
...
#define NUM_LEDS 120         // кол-во светодиодов для расчёта ограничения тока (только для диодов 5050!!!)
...
// ------ ИК пульт ------
#define USE_IR 0            // 1 - использовать ИК пульт, 0 - нет
#define IR_STEP 10          // шаг изменения настроек с пульта

// ------ Энкодер ------
#define USE_ENC 1          // 1 - использовать энкодер, 0 - нет
#define ENC_REVERSE 0       // 0 - инвертировать направление энкодера
#define ENC_TYPE 0          // 0 или 1 - тип энкодера
...
#define RESET_VAL 20
#define PRESET_ADDR 300
#define PRESET_START 20
...
const byte setAmount[] = {5, 4, 3, 3, 3, 4, 4, 5, 5, 5, 3, 5};         // количество настроек в режиме
const byte startFrom[] = {0, 4, 7, 9, 11, 13, 16, 19, 23, 27, 31, 33}; // начало индекса в списке лейблов
...
const char *modeNames[] = {    // список названий режимов
  ...
  "Fire", "FireM", "Strobe", "StrobeR", "Police", "Flash",
};
const int maxVals[] = {
  255, 255, 255, 255,
  255, 255, 255,
...
void setup() {
  ...
  if (eeprom_read_byte((uint8_t*)EEPR_RESET) != RESET_VAL) {    // первый запуск
    ...
    for (byte i = 0; i < MODE_AMOUNT; i++) eeprom_write_byte(i, 0);
    for (byte i = PRESET_START; i < 300; i += 2) {
