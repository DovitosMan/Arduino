...
#define sleep_timer 60000         // таймер сна в секундах
#define vape_threshold 10       // отсечка затяжки, в секундах
...
#define battery A7      // пин измерения напряжения акума
//-----------пины-------------
...
void setup() {
  ...
  if (battery_info) {  // отобразить заряд аккумулятора при первом включении
    disp.print(BVOL);
    delay(500);
	disp.printFloat((float)bat_vol/1000, 2);
    delay(1000);
    disp.print(BLANK);
  }
}
...
void loop() {
  ...
  if (flag) {                              // если акум заряжен
    ...
    if (set_hold && set_state) {           // если кнопка всё ещё удерживается
      ...
      if (round(millis() / 150) % 2 == 0) {
        if (!battery_percent)
			disp.printFloat((float)bat_volt_f/1000, 2); // показать заряд акума в вольтах
        else {

            disp.showNumberDec(map(bat_volt_f, battery_low * 1000, 4200, 0, 100)); // показать заряд акума в процентах
			}
      }
    }
    ...
    if (mode == 0 && !vape_state && !set_hold) {
      ...
      //---------кнопка ВНИЗ--------
	  disp.printFloat((float)volts/1000, 2); // отобразить на дисплее
    }
    // ------------------режим ВАРИВОЛЬТ-------------------
    ...
    if (mode == 1 && !vape_state && !set_hold) {
      ...
      //---------кнопка ВНИЗ--------
	  disp.printFloat(watts); // отобразить на дисплее
    }
    // ------------------режим ВАРИВАТТ--------------
    ...
    if (mode == 2 && !vape_state && !set_hold) {
      ...
      //---------кнопка ВНИЗ--------
	  disp.printFloat(ohms, 2);		// отобразить на дисплее
    }
    // ----------режим установки сопротивления-----------
    ...
    if (vape_state && flag && !wake_up_flag) {
      ...
      if (vape_mode == 1) {                                           // обычный режим парения
        (round(millis() / 150) % 2 == 0)?disp.print(vape1):disp.print(vape2);		// мигать медленно
	...
	Timer1.pwm(mosfet, PWM_f);                                    // управление мосфетом
      ...
      if (vape_mode == 2 && turbo_mode) {                             // турбо режим парения (если включен)
        (round(millis() / 50) % 2 == 0)?disp.print(vape1):disp.print(vape2);	// мигать быстро
  ...
  if (millis() - wake_timer > sleep_timer) {  // если кнопки не нажимались дольше чем sleep_timer секунд
    good_night();
  }
} // конец loop
  ...
  while (1) {
    ...
    if (vape_state && !vape_btt_f) {
      ...
      switch (click_count) {
        case 1: disp.print(V);
          break;
        case 2: disp.print(VA);
          break;
        case 3: disp.print(VAP);
          break;
        case 4: disp.print(VAPE);
          break;
      }
