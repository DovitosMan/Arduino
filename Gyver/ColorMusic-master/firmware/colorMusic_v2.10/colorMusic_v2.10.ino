...
// ----- настройки ленты
#define NUM_LEDS 322       //322 количество светодиодов (данная версия поддерживает до 410 штук)
#define CURRENT_LIMIT 40000  // лимит по току в МИЛЛИАМПЕРАХ, автоматически управляет яркостью (пожалей свой блок питания!) 0 - выключить лимит
byte BRIGHTNESS = 255;      // яркость по умолчанию (0 - 255)
#define BRIGHTNESSMAX 255

// ----- пины подключения
#define SOUND_R A14         // аналоговый пин вход аудио, правый канал
#define SOUND_L A13         // аналоговый пин вход аудио, левый канал
#define SOUND_GRND A10      // аналоговый пин земли аудио
#define SOUND_R_FREQ A15    // аналоговый пин вход аудио для режима с частотами (через кондер)
...
#define POT_GND 3              // пин земля для потенциометра
#define IR_PIN 20                // пин ИК приёмника
...
#define MAIN_LOOP 10               // период основного цикла отрисовки (по умолчанию 5)
...
float SMOOTH = 0.15 ;               // коэффициент плавности анимации VU (по умолчанию 0.5)
#define MAX_COEF 2.5              // коэффициент громкости (максимальное равно срднему * этот коэф) (по умолчанию 1.8)
...
#define HIGH_COLOR HUE_AQUA       // цвет высоких
...
#define STROBE_COLOR HUE_AQUA   // цвет стробоскопа
#define STROBE_SAT 100              // насыщенность. Если 0 - цвет будет БЕЛЫЙ при любом цвете (0 - 255)
...
void setup() {
  ...
  pinMode(SOUND_GRND, OUTPUT);
  digitalWrite(SOUND_GRND, LOW);
  ...
  switch (this_mode) {
    ...
    case 5:
      if (strobe_bright > 0)
        ...
        for (int i = 0; i < NUM_LEDS; i++) leds[i] = CHSV(EMPTY_COLOR, 0, EMPTY_BRIGHT);
      break;
    case 6:
      switch (light_mode) {
...
int smartIncrL(int value, int incr_step, int mininmum, int maximum) {
  int val_buf = value + incr_step;
  if (val_buf < mininmum)
    val_buf = maximum;
  if (val_buf >  maximum)
    ...
    switch (IRdata) {
      ...
      case BUTT_LEFT:
        if (settings_mode) {
          ...
          BRIGHTNESS = smartIncr(BRIGHTNESS, -20, 0, BRIGHTNESSMAX);
          FastLED.setBrightness(BRIGHTNESS);
        } else {
          switch (this_mode) {
            case 0:
            ...
            case 4: SMOOTH_FREQ = smartIncrFloat(SMOOTH_FREQ, -0.05, 0.05, 1);
              break;
            case 5: STROBE_SMOOTH = smartIncr(STROBE_SMOOTH, -10, 0, 255);
              break;
            case 6:
              switch (light_mode) {
                case 0: LIGHT_COLOR = smartIncrL(LIGHT_COLOR, -10, 0, 240);
                  break;
